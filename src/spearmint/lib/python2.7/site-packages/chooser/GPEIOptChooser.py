...
class GPEIOptChooser:
    ...
    def next(self, grid, values, durations,
        ...
        if self.mcmc_iters > 0:
            ...
            eis = np.mean(overall_ei, axis=1)
            best_cand = np.argmax(eis)

            if (best_cand >= numcand):
                return (int(numcand), cand[best_cand,:]), eis[best_cand]

            return int(candidates[best_cand]), eis[best_cand]
            ...
            return int(candidates[best_cand]), ei[best_cand]
