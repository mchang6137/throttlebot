...
def get_container_network_capacity(ssh_client):
        #Get the max capacity of the interface
        container_to_capacity = {}
        container_names = get_container_id(ssh_client, full_id=False, append_c=True)
        for container_name in container_names:
                ...
                max_capacity = 566
                try:
                        container_to_capacity[container_name] = float(max_capacity)
                except:
                        continue
...
def get_network_capabilities(ssh_client, stress_level):
        #hardcoding because we're pressed on time!
	container_to_capacity = get_container_network_capacity(ssh_client)
