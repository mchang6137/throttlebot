...
def set_cpu_quota(ssh_client, container_id, cpu_period, cpu_quota_percent):
    ...
    ssh_exec(ssh_client, update_command, contains_container_id=True)
...
def set_cpu_cores(ssh_client, container_id, cores):
    ...
    ssh_exec(ssh_client, set_cores_cmd, contains_container_id=True)
...
def reset_cpu_cores(ssh_client, container_id):
    ...
    ssh_exec(ssh_client, rst_cores_cmd, contains_container_id=True)
...
def set_container_blkio(ssh_client, container_id, disk_bandwidth):
    ...
    ssh_exec(ssh_client, set_cgroup_write_rate_cmd, contains_container_id=True)
    ssh_exec(ssh_client, set_cgroup_read_rate_cmd, contains_container_id=True)
...
def set_memory_size(ssh_client, container_id, memory_alloc):
    ...
    ssh_exec(ssh_client, set_memory_command, contains_container_id=True)

def reset_memory_size(ssh_client, container_id):
    reset_memory_command = 'docker update --memory=0 {}'.format(container_id)
    ssh_exec(ssh_client, reset_memory_command, contains_container_id=True)
