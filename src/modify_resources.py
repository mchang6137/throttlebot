...
quilt_machines = ["quilt", "machines"]

connected_pattern = re.compile(".+Connected}$")
ip_pattern = '(?<=PublicIP=)[\d|\.]*'
...
def remove_all_network_manipulation(ssh_client, remove_all_machines=False):
        ...
        if remove_all_machines is False:
                ...
                for machine in all_machines:
	                ...
	                container_ids = get_container_id(temp_ssh_client, full_id=False, append_c=True)
	                for container in container_ids:
		                cmd = "sudo tc qdisc del dev {} root".format(container)
...
def set_cpu_shares(ssh_client, cpu_quota):
        ...
        for container_name in container_names:
                if container_name not in container_blacklist:
                        update_command = 'docker update --cpu-period {} --cpu-quota {} {}'.format(cpu_period, cpu_quota, container_name)
...
def reset_cpu_shares(ssh_client):
	...
	cpu_quota = -1
        ...
        for container_name in container_names:
                if container_name not in container_blacklist:
                        update_command = 'docker update --cpu-quota -1 {}'.format(container_name)
                        ssh_client.exec_command(update_command)
...
def get_all_machines():
	...
	lines = p.stdout.read().splitlines()
	for line in lines:
		m  = re.search(ip_pattern, line).group(0)
