...
def collect_percentiles(traffic_client, filename):
    ...
    for percentile in percentiles:
        ...
        result_float = 0
        try:
            result_str = perc_results.read().decode("utf-8")
            print(result_str)
...
def execute_parse_results(ssh_client, cmd):
    ...
    try:
        results_str = results.read().decode("utf-8")
...
def measure_ml_matrix(workload_configuration, experiment_iterations):
    ...
    for x in range(experiment_iterations):
        ...
        result_time = runtime.read().decode("utf-8")
        print(result_time)
        try:
...
def measure_TODO_response_time(workload_configuration, iterations):
    ...
    for x in range(iterations):
        ...
        all_requests['latency'].append(execute_parse_results(traffic_client, latency_overall_cmd))
        ...
        try:
            print(("Successfully cleared? {}".format(cleared.read().decode("utf-8"))))
