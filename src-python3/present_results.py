...
def append_results_to_file(cpu, disk, network, resources, increments, experiment_type, iterations,
    ...
    with open(OUTPUT_DIRECTORY + output_file_name, 'wb') as csvfile:
        ...
        for service, data in base.items():
            all_metric_keys = list(data[min(increments)].keys())
            for metric in all_metric_keys:
                for increment_key in sorted(data.keys()):
...
def plot_results(data_file, resources, experiment_type, iterations, should_save, convertToMilli=True):
    ...
    for service, containerd in base.items():
        for container, data in containerd.items():
            for metric in data.keys():
                ...
                for increment_key in sorted(data[metric].keys()):
                    # Iterate through different metrics
                    if 'CPU' in resources:
