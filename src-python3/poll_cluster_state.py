...
def get_actual_vms():
    ...
    combined_machine_info = list(zip(machine_roles, machine_ips))
    ips = []
...
def get_service_placements(vm_ips):
    ...
    for vm_ip in vm_ips:
        ...
        container_ids = stdout1.read().decode("utf-8").splitlines()
        _, stdout2, _ = ssh_client.exec_command(docker_container_image_cmd)
        service_names = stdout2.read().decode("utf-8").splitlines()

        zipped_name_id = list(zip(service_names, container_ids))
...
def get_vm_to_service(vm_ips):
    ...
    for vm_ip in vm_ips:
        ...
        service_names = stdout1.read().decode("utf-8") .splitlines()
