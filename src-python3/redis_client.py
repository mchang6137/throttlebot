...
def get_top_n_mimr(redis_db, experiment_iteration_count, perf_metric, stress_weight, gradient_mode, optimize_for_lowest=True, num_results_returned=-1):
    ...
    for mr_result in mr_score_list:
        ...
        mr = generate_mr_from_hashkey(redis_db, mr_hash.decode("utf-8"))
...
def get_top_n_filtered_results(redis_db,
    ...
    str_pipeline_score_list = []
    for pipeline_score in pipeline_score_list:
        pipeline_repr,score = pipeline_score
        str_pipeline_score_list.append((pipeline_repr.decode("utf-8"), score))

    return str_pipeline_score_list
...
def read_summary_redis(redis_db, experiment_iteration_count):
    ...
    if mimr is not None:
        mimr = mimr.decode("utf-8")
    perf_improvement = redis_db.hget(hash_name, 'perf_improvement').decode("utf-8")
    action_taken = redis_db.hget(hash_name, 'action_taken').decode("utf-8")
    current_perf = redis_db.hget(hash_name, 'current_perf').decode("utf-8")
    elapsed_time = redis_db.hget(hash_name, 'elapsed_time').decode("utf-8")
    cumulative_mr = redis_db.hget(hash_name, 'cumulative_mr').decode("utf-8")
    analytic_perf = redis_db.hget(hash_name, 'analytic_perf').decode("utf-8")
...
def read_service_locations(redis_db, service):
    ...
    new_ip_list = []
    for ip in ip_list:
        new_ip_list.append(ip.decode("utf-8"))
    docker_list = redis_db.lrange(service_docker_key, 0, -1)
    
    new_docker_list = []
    for docker in docker_list:
        new_docker_list.append(docker.decode("utf-8"))

    return list(zip(new_ip_list, new_docker_list))
